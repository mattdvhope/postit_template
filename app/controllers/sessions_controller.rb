class SessionsController < ApplicationController
  ...
  # user = User.where(username: params[:username]).first is an alternative way to get a 'user' object
    if user && user.authenticate(params[:password]) # '&&' here means that if 'user' fails, then don't do the next thing (user.authenticate...); we're 'shore-circuiting' here
      if user.two_factor_auth? # <-- method in 'user.rb' which checks to see whether the user has a phone number
        session[:two_factor] = true #this prevents someone from typing '/pin' into the URL and trying to punch in 6-digit numbers; this says that a user must be in the session in order to access that page('/pin')
        # generate a pin
        ...
        redirect_to pin_path #this 'pin_path' helper method was invoked in 'routes.rb'; this goes to the 'pin.html.erb' template
      else
        login_user!(user)
      end
    else
  ...
  def pin #in 'routes.rb', this method must be specifically delineate for a 'post' request, but it is not necessary (just assumed to be there) by the 'get' request (the 'get' request simply renders the 'pin.html.erb' template); since we do have a 'post' request, we need this method here
