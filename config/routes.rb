PostitTemplate::Application.routes.draw do
  ...
  # below.. manual routes without resources (i.e. models)
  ...
  get '/login', to: 'sessions#new' #with these 'sessions' we're creating a resourceful entity called 'sessions' in order to have access to 'new', 'create', 'destroy' methods, but without having to create a model; we can thus use non-model-backed helper methods in the new.html.erb template; even though these are non-model-backed, we're trying to stick with RESTful naming conventions: 'new', 'create', etc.
  ...
  # below.. routes with resources (i.e. models)
  resources :posts, except: [:destroy] do
    ...
    resources :comments, only: [:create] do
      member do
        post :vote # 'post' is the HTTP verb; having this as post allows js in rails to operate in the views (i.e., in posts/_post.html.erb wherewith method: 'post' triggers the js in rails for data-method="post" in the html).
      end
    end
  end
